import type { LoginResponse, RegisterResponse, ForgotPasswordResponse } from '~~/server/types/auth/response.types'
import type { LoginRequest, RegisterRequest, ForgotPasswordRequest } from '~~/server/types/auth/request.types'
...
export const useAuth = () => {
  ...
  const forgotPassword = async (email: string) => {
    try {
      ...
      const response = await $fetch<AuthApi<ForgotPasswordResponse>>('/api/v1/auth/forgot-password', {
        method: 'POST',
  ...
  const resetPassword = async (accessToken: string, refreshToken: string, newPassword: string) => {
    try {
      ...
      return { 
        data: { message: 'Password has been successfully reset. You can now log in with your new password.' }, 
        error: null 
      }
    } catch (error: unknown) {
