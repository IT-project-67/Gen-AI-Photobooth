import type { ApiError } from "~~/server/types/error";

export const useApiError = () => {
  // Format error message for display
  const formatError = (error: ApiError | string): string => {
  ...
  const isAuthError = (error: ApiError): boolean => {
    if (!error || !error.code) return false;
    if (error.code.includes("AUTH") || error.code.includes("CREDENTIALS"))
      return true;
    return false;
  };
