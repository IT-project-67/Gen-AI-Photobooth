import type { UploadFile, ValidationOptions, FilePart } from "~~/server/types/storage";
import { ALLOWED_TYPES, ALLOWED_EXTS, MAX_FILE_SIZE, MIME_MAP } from "~~/server/types/storage";
import { createErrorResponse } from "~~/server/utils/core";
import { ERROR_STATUS_MAP } from "~~/server/types/core";

export function inferMimeFromFilename(filename: string, fallback = "image/jpeg"): string {
...
export function validateFileExtension(filename: string, allowedExts: string[]): void {
  const extLower = getExtLower(filename);

  if (!extLower) {
    throw createH3ValidationError("INVALID_FILE_NAME", "File must have a valid extension");
  }
...
export function validateFileOrThrow(file: UploadFile, options: ValidationOptions = {}): void {
