...
import { handleAuthError, handleApiError } from '../../../utils/auth/error-handler.utils'
...
export default defineEventHandler(async (event): Promise<ApiResponse<ResetPasswordResponse>> => {
  try {
    // Get request body
    const body = await readBody(event) as ResetPasswordRequest

    // Validate input
    const validationError = validateResetPasswordRequest(body.access_token, body.refresh_token, body.password)
    if (validationError) {
      throw createError({
